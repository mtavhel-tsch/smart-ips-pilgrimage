		...
		<xsl:for-each  select="$innerActors[./bpmn:flowNodeRef = $linkSrc][1]">
		  ...
		  <xsl:for-each  select="$innerActors[./bpmn:flowNodeRef = $linkTgt][1]">
			...
			<GraphDefinition>
			  ...
			  <name><xsl:attribute name="value"><xsl:value-of select="$processName"/></xsl:attribute></name>
			  <description><xsl:attribute name="value"><xsl:value-of select="$linkDesc"/></xsl:attribute></description>
			  <status value="active"/>
			  <experimental value="false"/>
			  <node>
